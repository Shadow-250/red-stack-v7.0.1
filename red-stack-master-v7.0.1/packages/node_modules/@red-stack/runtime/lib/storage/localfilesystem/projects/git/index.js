...
function runGitCommandWithAuth(args,cwd,auth,emit) {
    ...
    return authResponseServer(auth).then(function(rs) {
        ...
        commandEnv.GIT_ASKPASS = path.join(__dirname,"red-stack-ask-pass.sh");
        commandEnv.RED_STACK_GIT_NODE_PATH = process.execPath;
        commandEnv.RED_STACK_GIT_SOCK_PATH = rs.path;
        commandEnv.RED_STACK_GIT_ASKPASS_PATH = path.join(__dirname,"authWriter.js");
...
function runGitCommandWithSSHCommand(args,cwd,auth,emit) {
    ...
    return sshResponseServer(auth).then(function(rs) {
        var commandEnv = clone(process.env);
        commandEnv.SSH_ASKPASS = path.join(__dirname,"red-stack-ask-pass.sh");
        commandEnv.DISPLAY = "dummy:0";
        commandEnv.RED_STACK_GIT_NODE_PATH = process.execPath;
        commandEnv.RED_STACK_GIT_SOCK_PATH = rs.path;
        commandEnv.RED_STACK_GIT_ASKPASS_PATH = path.join(__dirname,"authWriter.js");
        // For git < 2.3.0
        commandEnv.GIT_SSH = path.join(__dirname,"red-stack-ssh.sh");
        commandEnv.RED_STACK_KEY_FILE=auth.key_path;
