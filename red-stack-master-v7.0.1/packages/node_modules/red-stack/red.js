...
if (parsedArgs.help) {
    console.log("RED-Stack v"+RED.version());
    console.log("Usage: red-stack [-v] [-?] [--settings settings.js] [--userDir DIR]");
    ...
    console.log("");
    console.log("Documentation can be found at homepage");
    process.exit();
}
...
process.env.RED_STACK_HOME = process.env.RED_STACK_HOME || __dirname;
...
if (parsedArgs.settings) {
    ...
    if (fs.existsSync(path.join(process.env.RED_STACK_HOME,".config.json"))) {
        // RED_STACK_HOME contains user data - use its settings.js
        settingsFile = path.join(process.env.RED_STACK_HOME,"settings.js");
    } else if (process.env.HOMEPATH && fs.existsSync(path.join(process.env.HOMEPATH,".red-stack",".config.json"))) {
        // Consider compatibility for older versions
        settingsFile = path.join(process.env.HOMEPATH,".red-stack","settings.js");
    } else {
        var userDir = parsedArgs.userDir || path.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH,".red-stack");
        var userSettingsFile = path.join(userDir,"settings.js");
        if (fs.existsSync(userSettingsFile)) {
            // $HOME/.red-stack/settings.js exists
...
if (parsedArgs.safe || (process.env.RED_STACK_ENABLE_SAFE_MODE && !/^false$/i.test(process.env.RED_STACK_ENABLE_SAFE_MODE) )) {
    settings.safeMode = true;
}
if (process.env.RED_STACK_ENABLE_PROJECTS) {
    settings.editorTheme = settings.editorTheme || {};
    settings.editorTheme.projects = settings.editorTheme.projects || {};
    settings.editorTheme.projects.enabled = !/^false$/i.test(process.env.RED_STACK_ENABLE_PROJECTS);
}
...
// replicate (settings.uiPort = parsedArgs.port||settings.uiPort||1999;) but allow zero
if (parsedArgs.port !== undefined){
    settings.uiPort = parsedArgs.port;
} else {
    if (settings.uiPort === undefined){
        settings.uiPort = 1999;
    }
}
...
try {
    ...
    if (err.code == "unsupported_version") {
        ...
        console.log("RED-Stack requires Node.js v8.9.0 or later");
    } else {
...
function basicAuthMiddleware(user,pass) {
    ...
    var checkPasswordAndCache = function(p) {
        ...
        // a restart of RED-Stack. This means we can cache the provided crypted
...
RED.start().then(function() {
    if (settings.httpAdminRoot !== false || settings.httpNodeRoot !== false || settings.httpStatic) {
        ...
        server.listen(settings.uiPort,settings.uiHost,function() {
            ...
            process.title = parsedArgs.title || 'red-stack';
