...
var util = require("@red-stack/util").util;
...
function getBasePath(config) {
    ...
    if (!config.dir) {
        if(config.settings && config.settings.userDir){
            ...
            try {
                fs.statSync(path.join(process.env.RED_STACK_HOME,".config.json"));
                storageBaseDir = path.join(process.env.RED_STACK_HOME, base);
            } catch(err) {
                try {
                    // Consider compatibility for older versions
                    if (process.env.HOMEPATH) {
                        fs.statSync(path.join(process.env.HOMEPATH,".red-stack",".config.json"));
                        storageBaseDir = path.join(process.env.HOMEPATH, ".red-stack", base);
                    }
                } catch(err) {
                }
                if (!storageBaseDir) {
                    storageBaseDir = path.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH || process.env.RED_STACK_HOME,".red-stack", base);
                }
            }
        }
    }else{
