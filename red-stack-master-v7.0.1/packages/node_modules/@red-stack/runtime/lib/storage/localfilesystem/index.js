...
var log = require("@red-stack/util").log; // TODO: separate module
...
var localfilesystem = {
    init: function(_settings, runtime) {
        ...
        if (!settings.userDir) {
            try {
                fs.statSync(fspath.join(process.env.RED_STACK_HOME,".config.json"));
                settings.userDir = process.env.RED_STACK_HOME;
            } catch(err) {
                try {
                    // Consider compatibility for older versions
                    if (process.env.HOMEPATH) {
                        fs.statSync(fspath.join(process.env.HOMEPATH,".red-stack",".config.json"));
                        settings.userDir = fspath.join(process.env.HOMEPATH,".red-stack");
                    }
                } catch(err) {
                }
                if (!settings.userDir) {
                    settings.userDir = fspath.join(process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH || process.env.RED_STACK_HOME,".red-stack");
        ...
        if (!settings.readOnly) {
            packagePromise = function() {
                try {
                    ...
                    var defaultPackage = {
                        "name": "red-stack-project",
                        "description": "A RED-Stack Project",
                        "version": "0.0.1",
                        "private": true
                    };
