...
<script type="text/x-red" data-help-name="file in">
    ...
    <h3>詳細</h3>
    <p>ファイルネームは絶対パスでの指定を推奨します。絶対パスを指定しない場合は、RED-Stackプロセスのワーキングディレクトリからの相対パスとして扱います。</p>
    ...
    <h4>旧式のエラー処理</h4>
    <p>RED-Stack 0.17より前の版では、ファイルの読み込み時にエラーが発生すると<code>payload</code>を持たず<code>error</code>プロパティにエラーの詳細情報を設定したメッセージを送信します。この動作モードは非推奨であり、新しいノード実装ではデフォルトでは無効としています。ノードの設定により、必要に応じてこのモードを有効にできます。</p>
