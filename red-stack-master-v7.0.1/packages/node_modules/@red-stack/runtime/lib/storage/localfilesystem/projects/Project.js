...
var settings;
var runtime;
var log = require("@red-stack/util").log;
        ...
        if (missingFiles.indexOf('package.json') === -1) {
            ...
            promises.push(fs.readFile(fspath.join(project.path,project.paths['package.json']),"utf8").then(function(content) {
                try {
                    ...
                    if (project.package.hasOwnProperty('red-stack')) {
                        if (project.package['red-stack'].hasOwnProperty('settings')) {
                            project.paths.flowFile = fspath.join(project.paths.root,project.package['red-stack'].settings.flowFile);
                            project.paths.credentialsFile = fspath.join(project.paths.root,project.package['red-stack'].settings.credentialsFile);
                        }
                    } else {
                        // TODO: package.json doesn't have a red-stack section
                        //       is that a bad thing?
                    }
                } catch(err) {
...
Project.prototype.update = function (user, data) {
    ...
    if (data.hasOwnProperty('files')) {
        this.package['red-stack'] = this.package['red-stack'] || { settings: {}};
        ...
        if (data.files.hasOwnProperty('flow') && this.package['red-stack'].settings.flowFile !== data.files.flow.substring(this.paths.root.length)) {
            this.paths.flowFile = data.files.flow;
            this.package['red-stack'].settings.flowFile = data.files.flow.substring(this.paths.root.length);
            savePackage = true;
            flowFilesChanged = true;
        }
        if (data.files.hasOwnProperty('credentials') && this.package['red-stack'].settings.credentialsFile !== data.files.credentials.substring(this.paths.root.length)) {
            this.paths.credentialsFile = data.files.credentials;
            this.package['red-stack'].settings.credentialsFile = data.files.credentials.substring(this.paths.root.length);
